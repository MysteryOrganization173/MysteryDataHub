...
const getBundleName = (bundle) => {
  const networkName = bundle.operator === 'airteltigo'
    ? 'AirtelTigo'
    : bundle.operator.charAt(0).toUpperCase() + bundle.operator.slice(1);
  const bundleType = bundle.isBigTime ? 'BigTime' : bundle.isInstant ? 'Instant' : 'Express';
  return `${networkName} ${bundle.size} ${bundleType}`;
};

const getDeliveryTime = (bundle) => {
  if (bundle.isInstant) return 'Instant';
  if (bundle.isBigTime) return '30min-4h';
  return '5-45min';
};

// Add required derived fields before insert
const bundles = bundlesRaw.map(b => ({
  ...b,
  name: getBundleName(b),
  wholesalePrice: getWholesalePrice(b.basePrice),
  deliveryTime: getDeliveryTime(b)
}));
...
const seed = async () => {
  try {
    ...
    if (!adminExists) {
      ...
      console.log(`✅ Admin user created (email: ${DEFAULT_ADMIN_EMAIL}, password: ${DEFAULT_ADMIN_PASSWORD})`);
    } else {
...
seed();
>>>>>>> 17650113d203b4fa03334b3c02a071453522cc77
